...
from gyatt_o_tune.ui.preferences_dialog import (
    IdentifierExpressionLineEdit,
    RowVisualizationPreferencesDialog,
    TableLogChannelPreferencesDialog,
    load_preferences,
...
class MainWindow(QMainWindow):
    ...
    def _load_table_log_channel_preferences(self) -> None:
        tables = load_preferences()
        self.table_log_channel_preferences = tables
        if "__show_tunerstudio_names__" in tables:
            self.show_tunerstudio_names = bool(tables["__show_tunerstudio_names__"])
        ...
        _filter_state = tables.get("__table_filter_state__", "")
        if _filter_state == "favorites":
            self.only_show_favorited_tables = True
            self.show_1d_tables = False
            self.show_2d_tables = False
        elif _filter_state == "2d":
            self.only_show_favorited_tables = False
            self.show_1d_tables = False
            self.show_2d_tables = True
    ...
    def _save_table_log_channel_preferences(self) -> None:
        tables = dict(self.table_log_channel_preferences)
        tables["__show_tunerstudio_names__"] = self.show_tunerstudio_names
        tables["__favorite_tables__"] = sorted(self.favorite_tables)
        if self.only_show_favorited_tables:
            tables["__table_filter_state__"] = "favorites"
        elif self.show_2d_tables:
            tables["__table_filter_state__"] = "2d"
        else:
            tables["__table_filter_state__"] = "1d"
        save_preferences(tables)
    ...
    def _open_preferences(self, initial_table_name: str | None = None, initial_scatter_identifier: str | None = None) -> None:
        ...
        dialog = TableLogChannelPreferencesDialog(
            ...
            current_preferences_path=preferences_path(),
        ...
        if isinstance(loaded_favorites, list):
            self.favorite_tables = {str(name) for name in loaded_favorites if isinstance(name, str) and str(name).strip()}
            self._save_favorites()
        # Apply table filter state from the dialog back to the main window.
        _filter_state = self.table_log_channel_preferences.get("__table_filter_state__", "")
        if _filter_state == "favorites":
            self.only_show_favorited_tables = True
            self.show_1d_tables = False
            self.show_2d_tables = False
    ...
    def _create_layout(self) -> None:
        ...
        # Single click should only open the row (do not focus editor). Double-click opens editor.
        self.table_grid.cellClicked.connect(self._on_table_grid_cell_clicked)
        self.table_grid.cellDoubleClicked.connect(self._on_table_grid_cell_double_clicked)
    ...
    def _on_table_grid_cell_clicked(self, row: int, column: int) -> None:
        # Treat single click like a row selection: load the row but do not focus or open
        # the row editor or select an editor cell. For 1D tables, always load the
        # single logical row (index 0).
        if self._is_current_table_1d():
            # In 1D tables the selected row is the full table.
            self._load_selected_table_row(0, preferred_column=None, select_editor_cell=False)
            return
